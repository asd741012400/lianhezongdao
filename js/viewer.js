...
(function (factory) {
  ...
  $.extend(prototype, {
    ...
    view: function (index) {
      ...
      this.index = index;
      this.image = null;
      this.$canvas.html("<div style='text-align:center;height:50px;line-height:50px;color: #FFFFFF;position: fixed;width: 200px;left: 50%;margin-left:-100px;top: 40%;font-size: 14px;'>图片正在加载中...</div>");
      var _thisCanvas = this.$canvas;
      $image.one(EVENT_LOAD, $.proxy(this.load, this));
      $image[0].onload = function(){
        _thisCanvas.html($image);
      }
      $title.empty();
